                ...
                with tf.Graph().as_default():
                    ...
                    if preprocessing_method == 'outlier':
                        ...
                        path_sorted_list = preprocessing.outlier(sess, x, linear_weights, linear_dict, preds, model,
                                                                 sensitive_param,
                                                                 data_config[dataset].input_bounds[sensitive_param - 1][
                                                                     0],
                                                                 data_config[dataset].input_bounds[sensitive_param - 1][
                                                                     1], X, nb_classes, Y)
                        # loaded_X, loaded_Y, loaded_input_shape = sorted_data.read_dataset(dataset, sen,
                        #                                                                   'top200_sorted_dataset')
                        # assert loaded_X.shape[1] == X.shape[1]
                        # assert loaded_Y.shape[1] == Y.shape[1]
                        # assert loaded_input_shape == input_shape
                        # X = loaded_X
                        # Y = loaded_Y
                        # input_shape = loaded_input_shape
                        # X_backup = np.copy(loaded_X)
                    ...
                    for num in range(len(inputs)):
                        ...
                        for iter in range(max_iter + 1):
                            ...
                            if label != n_label and (tuple(temp) not in global_disc_inputs) and (
                                ...
                                local_perturbation = Local_Perturbation(sess, x, nx, x_grad, nx_grad, n_value,
                                                                        sensitive_param, input_shape[1],
                                                                        data_config[dataset])
                            ...
                            s_grad, n_grad, sn_grad = sess.run(
                                [tf.sign(x_grad), tf.sign(nx_grad), tf.sign(x_grad + nx_grad)],
                    ...
                    RQ2_table.append(
                        [len(global_disc_inputs), len(local_disc_inputs), tot_disc, local_succ, time_to_1st,
                         time_to_1000])
