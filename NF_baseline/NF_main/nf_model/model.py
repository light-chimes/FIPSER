...
class Model(object):
    ...
    def get_all_activation(self, x, only_hidden=True, with_dict=False):
        output = self.fprop(x)
        activations = []
        dicts = {}
        for i in output:
            if 'ReLU' in i:
                activations.append(output[i])
            if with_dict:
                if only_hidden and 'ReLU' in i:
                    dicts[i] = output[i]
                elif not only_hidden:
                    dicts[i] = output[i]
        if with_dict:
            return activations, dicts
        return activations
