...
from baseline.NF_main.src import preprocessing
...
def pre_arg(**kwargs_a):
    def wrapper(func):
        def wrapper(*args, **kwargs_b):
            return func(*args, **{**kwargs_a, **kwargs_b})
...
class Local_Perturbation(object):
    ...
    def __call__(self, x):
        ...
        if self.update_interval <= self.update_counter or self.pre_prob is None:
            if self.pre_prob is not None:
                self.pre_pre_prob = self.pre_prob
            ...
            self.pre_prob = probs
            if self.pre_pre_prob is None:
                self.pre_pre_prob = self.pre_prob
        self.update_counter += 1
        # randomly choose the feature for local perturbation
        index = np.random.choice(range(self.input_shape), p=self.pre_prob)
...
def dnn_fair_testing(dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter,
    ...
    data = {"census": census_data, "credit": credit_data, "bank": bank_data, "compas": compas_data,
            "default": default_data, "heart": heart_data, "diabetes": diabetes_data,"meps15": meps15_data, "meps16": meps16_data,
    ...
    for dataset in data.keys():
        ...
        global time_to_1st
        global time_to_1000
        # preprocessing_method = 'cluster'
        # preprocessing_method = 'random'
        preprocessing_method = 'outlier'
        ...
        for sen in sens_p:
            ...
            for trial in range(1):
                ...
                if preprocessing_method == 'outlier':
                    x_all_hidden, act_dict = model.get_all_activation(x, with_dict=True, only_hidden=False)
                    linear_dict = {}
                    for i in act_dict:
                        if 'Linear' in i:
                            linear_dict[i] = act_dict[i]
                    path_sorted_list = preprocessing.outlier(sess, x, linear_weights, linear_dict, preds, model,
                                                             sensitive_param,
                                                             data_config[dataset].input_bounds[sensitive_param - 1][0],
                                                             data_config[dataset].input_bounds[sensitive_param - 1][1],
                                                             X, nb_classes,Y)
                    X = X[path_sorted_list]
                    Y = Y[path_sorted_list]
                elif preprocessing_method == 'random':
                    np.random.shuffle(X)
                    np.random.shuffle(Y)
                ...
                for num in range(len(inputs)):
                    ...
                    for iter in range(max_iter + 1):
                        ...
                        if label != n_label and (tuple(temp) not in global_disc_inputs) and (
                            ...
                            local_perturbation = Local_Perturbation(sess, grad_0, x, n_value, sensitive_param,
                                                                    input_shape[1],
                                                                    data_config[dataset])

                            @pre_arg(perturbator=local_perturbation)
                            def evaluate_local(inp, **kwargs):
                                """
                                ...
                                if result and (tuple(temp) not in global_disc_inputs) and (
                                        tuple(temp) not in local_disc_inputs):
                                    if time_to_1st == 0:
                                        time_to_1st = round(time.time() - time1, 4)
                                ...
                                if not result:
                                    kwargs['perturbator'].update_counter = 0
                                    kwargs['perturbator'].pre_pre_prob = kwargs['perturbator'].pre_prob
                            ...
                            print(len(local_disc_inputs_list),
                                  ...
                                  'time cost:', time.time() - begin_time, 'current time:', datetime.datetime.now())
                            break
                        ...
                        # global perturbation
                        s_grad = s_grad * global_decay + sess.run(tf.sign(grad_0), feed_dict={x: sample})
                        n_grad = n_grad * global_decay + sess.run(tf.sign(grad_0), feed_dict={x: n_sample})
                        ...
                        sample[0] = clip(sample[0] + perturbation_size * cal_grad[0], data_config[dataset]).astype(
                            "int")
                ...
                np.save(
                    '../results/' + dataset + '/ADF/' + str(sensitive_param) + '/ADF_suc_idx_' + str(trial) + '.npy',
                    np.array(suc_idx))
                np.save('../results/' + dataset + '/ADF/' + str(sensitive_param) + '/ADF_global_samples_' + str(
                    trial) + '.npy', np.array(global_disc_inputs_list))
                np.save('../results/' + dataset + '/ADF/' + str(sensitive_param) + '/ADF_local_samples_' + str(
                    trial) + '.npy', np.array(local_disc_inputs_list))
                np.save(
                    '../results/' + dataset + '/ADF/' + str(sensitive_param) + '/ADF_disc_value_' + str(trial) + '.npy',
                    np.array(value_list))
                ...
                RQ2_table.append(
                    [len(global_disc_inputs), len(local_disc_inputs), tot_disc, local_succ, time_to_1st, time_to_1000])
