...
def outlier(sess, x, linear_weights, linear_dict, preds, model, sens_param, sens_attr_lower_bound,
    ...
    def get_feature_weights(net, sample, linears_weight):
        ...
        feature_distances = distance.cdist(all_origin_features[:, :-acts[-1].shape[1]],
                                           all_origin_features[:, :-acts[-1].shape[1]],
                                           'euclidean')
        feature_distances[feature_distances == 0] = np.max(feature_distances)
        nearest_line = np.argmin(feature_distances, axis=1)
        differents = all_origin_features - all_origin_features[nearest_line]
        differents_features = differents[:, :-acts[-1].shape[1]]
        differents_labels = differents[:, -acts[-1].shape[1]:]

        all_origin_features = all_origin_features[:, :-acts[-1].shape[1]]

        sum_impacts = np.zeros_like(differents_features)
        for i in range(acts[-1].shape[1]):
            current_label = differents_labels[:, i].reshape((-1, 1))
            current_impact = current_label / differents_features
            current_impact[differents_features == 0] = 0
            current_impact = np.abs(current_impact)
            sum_impacts += current_impact
        impacts = np.mean(sum_impacts, axis=0)
        ...
        weighted_score = grad_potent[:, sens_param-1]
        ...
        return importance, sorted_idx, acts, short_grad_potent, all_importance, weighted_score, impacts, all_origin_features, dropout_importance

    feature_weight, sidx, acts, short_grad_potent, all_importance, potent_score, impacts, all_origin_features, dropout_importance = get_feature_weights(
        sess, X,
        linear_weights)
